...
async def get_review_from_db(user_id:UUID, film_id:UUID):
...
async def create_review( user_id, film_id, review_value):
    review = await get_review_from_db(user_id=user_id, film_id=film_id)
    if review:
        raise HTTPException(
...
async def update_review( user_id, film_id, review_value):
    review = await get_review_from_db(user_id=user_id, film_id=film_id)
    if not review:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
